...
static int gpio_keys_suspend(struct device *dev)
{
	...
	struct gpio_keys_platform_data *pdata = pdev->dev.platform_data;
	int i;
//				printk("***************gpio_keys_suspend1*********************\n");
	if (device_may_wakeup(&pdev->dev)) {
//	printk("***************gpio_keys_suspend2*********************\n");
		for (i = 0; i < pdata->nbuttons; i++) {
//		printk("***************gpio_keys_suspend3*********************\n");
			struct gpio_keys_button *button = &pdata->buttons[i];
//			printk("***************gpio_keys_suspend4*********************\n");
			if (button->wakeup) {
//			printk("***************gpio_keys_suspend5*********************\n");
				int irq = gpio_to_irq(button->gpio);
//				printk("***************gpio_keys_suspend6*********************\n");
//				enable_irq_wake(irq);
				enable_irq_wake(s3c_gpio_keys[i].eint);
//				printk("***************gpio_keys_suspend7*********************\n");
			}
		}
	}

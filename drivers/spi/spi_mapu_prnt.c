...
{
	printk("nisp: mapu_spi_xfer_is_last\n");
...
{
	printk("nisp: mapu_spi_xfer_can_be_chained\n");
				...
				struct spi_message *msg)
{
	...
	printk("nisp: spi_xfer loop=%d, remaining=%d\n", loop, remaining);
				...
				struct spi_message *msg)
{
	...
	printk("nisp: mapu_spi_next_xfer\n");
...
static void mapu_spi_next_message(struct spi_master *master)
{
	...
	printk("nisp: mapu_spi_next_message\n");
...
mapu_spi_interrupt(int irq, void *dev_id)
{
	...
	printk("nisp: mapu_spi_interrupt\n");
...
static int mapu_spi_transfer(struct spi_device *spi, struct spi_message *msg)
{
	...
	spin_unlock(&ms->lock);
	printk("nisp: mapu_spi_transfer done\n");
	printk("****************************\n");
	return 0;
}
...
void mapu_spi_hw_init(void __iomem *regbase)
{
	...
	__raw_writel(0x6, regbase + MAPU_SPI_BAUDR);
